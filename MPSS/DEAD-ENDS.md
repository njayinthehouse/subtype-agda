...
| 32 | an input promotion node consumed by two pulls along one path | `distinct-probe.py`, `../PLAN.md` "Two more structural facts" | **not a dead end**: never happens in 600,000 pulls at depths 2–5, including configurations built for it. If proved, the recursion is bounded by the number of promotion nodes in the inputs and the diamond follows | the proof of distinct consumption; for context variables it is the no-repeat property (row 29) |
