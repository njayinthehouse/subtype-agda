"""Along a path, is the same input promotion node (tree, position) ever consumed twice?"""
...
def run(k,kc,maxruns,sel=None):
    ...
    def rec(d1,d2,c1,c2,path):
        ...
        if (r1,r2) in (('Var','Pro'),('Pro','Var')):
            partner=d2 if r1=='Var' else d1
            node=('EV',P.pos(partner))
            seen=[e for e in path if isinstance(e,tuple) and e and e[0]=='EV']
            if node in seen:
                STATS['REPEATED-PRO-NODE']+=1
                if len(EX)<5: EX.append((S.show(partner['src']),S.showG(partner['G']),S.showS(partner['s']),node[1]))
            else: STATS['distinct']+=1
            return orig(d1,d2,c1,c2,path+(node,))
